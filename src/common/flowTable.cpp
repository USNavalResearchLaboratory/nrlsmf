...
FlowDescription::FlowDescription(const ProtoAddress&  dst,            // invalid dst addr means any dst
                             ...
                             unsigned int            ifaceIndex)
{
    ...
    if (keylen > KEY_MAX)
    {
        PLOG(PL_ERROR, "FlowDescription::SetKey() error: invalid address length\n");
        flow_keysize = 0;
        return;
    }
...
}  // end FlowDescription::SetKey()

void FlowDescription::InitFromDescription(const FlowDescription& description, int flags)
{
    // flow_key:  dlen [+ dst + dmask] + slen [+ src + smask] + class + protocol + index
    if (flags < FLAG_ALL)
    {
...
}  // end FlowDescription::InitFromDescription()

bool FlowDescription::InitFromPkt(ProtoPktIP& ipPkt, unsigned int ifaceIndex, int flags)
{
    // flow_key:  dlen [+ dst + dmask] + slen [+ src + smask] + class + protocol + index
    switch (ipPkt.GetVersion())
    {
        ...
        default:
            PLOG(PL_ERROR, "FlowDescription::InitFromPkt() error: invalid IP version!\n");
...
void FlowDescription::SetDstMaskLength(UINT8 dstMask)
{
...
}  // end FlowDescription::SetDstMaskLength()

void FlowDescription::SetSrcMaskLength(UINT8 srcMask)
{
    UINT8 srcLen = GetSrcLength();
...
}  // end FlowDescription::SetSrcMaskLength()

bool FlowDescription::GetDstAddr(ProtoAddress& addr) const
{
    // flow_key:  dlen [+ dst + dmask] + slen [+ src + smask] + class + protocol + index
    switch (GetDstLength())
    {
        ...
        case 16:
            ...
            return true;
        default:
            PLOG(PL_ERROR, "FlowDescription::GetDstAddr() error: invalid address length\n");
            return false;
    }
}  // end FlowDescription::GetDstAddr()

bool FlowDescription::GetSrcAddr(ProtoAddress& addr) const
{
    ...
    switch (GetSrcLength())
    {
        ...
        case 16:
            ...
            return true;
        default:
            PLOG(PL_ERROR, "FlowDescription::GetSrcAddr() error: invalid address length\n");
            return false;
    }
}  // end FlowDescription::GetSrcAddr()

FlowTable::MaskLengthList::MaskLengthList()
 : list_length(0)
{
...
void FlowTable::MaskLengthList::Insert(UINT16 value)
{
    if (value > MASK_SIZE_MASK)
    {
        PLOG(PL_ERROR, "FlowTable::MaskLengthList::Insert() error: invalid value\n");
        return;
    }
...
FlowTable::Entry::~Entry()
{
}

FlowTable::Iterator::Iterator(BaseIterator& tableIterator, MaskLengthList& maskLengthList)
 : table_iterator(tableIterator), mask_iterator(maskLengthList)
{
    // the derived class should call Reset() in its constructor
}

FlowTable::Iterator::~Iterator()
{
}
...
void FlowTable::Iterator::Reset(const FlowDescription* description, int flags)
{
    ...
    if (NULL != description)
    {
        ...
        if ((0 != dstLen) && (0 != (FlowDescription::FLAG_DST & flags)))
        {
            UINT8 dstMask = description->GetDstMaskLength();
        ...
        traffic_class = (0 != (FlowDescription::FLAG_CLASS & flags)) ? description->GetTrafficClass() : 0x03;
        if (extendPrefix)
        {
        ...
        protocol = (0 != (FlowDescription::FLAG_PROTO & flags)) ? description->GetProtocol() : ProtoPktIP::RESERVED;
        if (extendPrefix)
        {
            if (ProtoPktIP::RESERVED != protocol)
...
FlowTable::Entry* FlowTable::Iterator::GetNextEntry()
{
    ...
    return entry;
}  // end FlowTable::Iterator::GetNextEntry()

FlowTable::Entry* FlowTable::Iterator::FindBestMatch(const FlowDescription& flowDescription, bool deepSearch)
{
    ...
    return bestMatch;
}  // end FlowTable::Iterator::FindBestMatch()
