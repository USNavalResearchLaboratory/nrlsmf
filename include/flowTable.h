...
class FlowDescription
{
    public:
        ...
        // The FlowDescription key is organized so that prefix-based iterations
        ...
        FlowDescription(const ProtoAddress&  dst = PROTO_ADDR_NONE, // invalid dst addr means "null" dst
        ...
        void InitFromDescription(const FlowDescription& description, int flags = FLAG_ALL);
...
};  // end class FlowDescription

// The FlowTableTemplate class provides a _base_ class for
// data structures that have items with a FlowDescription.
class FlowTable
{
    public:
        class Entry
        {
            public:
                ...
                const FlowDescription& GetFlowDescription() const
    ...
    protected:
        ...
        class MaskLengthList
        {
            public:
                ...
                enum {MASK_SIZE_MASK = FlowDescription::KEY_MAX*8};
        ...
        // structure types that may be used for derived FlowTable classes.
        class BaseIterator
        {
            public:
        ...
        };  // end class FlowTable::BaseIterator
        ...
        class Iterator
        {
            ...
            protected:
                ...
                // this common iterator over FlowDescription entries.
                ...
                Entry* FindBestMatch(const FlowDescription& flowDescription, bool deepSearch=false);
            ...
            private:
                ...
                char                        prefix_mask[FlowDescription::KEY_MAX];
...
// Separate FlowEntryTemplate and FlowTableTemplate classes are provided 
...
class FlowEntryTemplate : public FlowTable::Entry, public TABLE_TYPE::Item
{
    public:
        FlowEntryTemplate(const ProtoAddress&  dst = PROTO_ADDR_NONE, // invalid dst addr means "null" dst
                          const ProtoAddress&  src = PROTO_ADDR_NONE, // invalid src addr means "null" src
                          UINT8                trafficClass = 0x03,   // "null" trafficClass 
                          ProtoPktIP::Protocol theProtocol = ProtoPktIP::RESERVED, // "null" protocol
                          unsigned int         ifaceIndex = 0)        // "null" interface index 
            : FlowTable::Entry(dst, src, trafficClass, theProtocol, ifaceIndex) {}
        FlowEntryTemplate(const FlowDescription& flowDescription, int flags = FlowDescription::FLAG_ALL)
            : FlowTable::Entry(flowDescription, flags) {}
        ~FlowEntryTemplate() {}
        
        const char* GetKey() const
            {return FlowTable::Entry::GetKey();}
        unsigned int GetKeysize() const
            {return FlowTable::Entry::GetKeysize();}
};  // end class FlowEntryTemplate

// "ENTRY_TYPE" MUST be of type derived from FlowEntryTemplate for same given "TABLE_TYPE"
template <class ENTRY_TYPE, class TABLE_TYPE>
class FlowTableTemplate : public FlowTable, public TABLE_TYPE
{ 
    private:
        class BaseIterator : public FlowTable::BaseIterator, public TABLE_TYPE::Iterator
        {
            public:
        ...
        };  // end class FlowTableTemplate::BaseIterator
    ...
    public:
        ...
        FlowTableTemplate() {};
        virtual ~FlowTableTemplate() {}
        
        bool InsertEntry(ENTRY_TYPE& entry)
        {
            if (TABLE_TYPE::Insert(entry))
            {
                FlowTable::Insert(entry.GetPrefixSize());
                return true;
            }
            return false;
        }
        ...
        {
            FlowTable::Remove(entry.GetPrefixSize());
            TABLE_TYPE::Remove(entry);
        }
        
        ENTRY_TYPE* FindEntry(const FlowDescription& flowDescription)
        ...
        ENTRY_TYPE* FindBestMatch(const FlowDescription& flowDescription, bool deepSearch=false)
        {
            FlowTableTemplate<ENTRY_TYPE, TABLE_TYPE>::Iterator iterator(*this, &flowDescription);
            return iterator.FindBestMatch(flowDescription, deepSearch);
        }
        
        class Iterator : public FlowTable::Iterator
        {
            public:
                ...
                int GetCurrentMaskLength() const
                    {return FlowTable::Iterator::GetCurrentMaskLength();}
                
            private:
                friend class FlowTableTemplate;
                ENTRY_TYPE* FindBestMatch(const FlowDescription& flowDescription, bool deepSearch=false)
                    {return static_cast<ENTRY_TYPE*>(FlowTable::Iterator::FindBestMatch(flowDescription, deepSearch));}
                
                FlowTableTemplate::BaseIterator    base_iterator;
            
        };  // end class FlowTableTemplate::Iterator
};  // end FlowTableTemplate
