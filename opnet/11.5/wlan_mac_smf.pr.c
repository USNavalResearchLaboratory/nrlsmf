/* Process model C form file: wlan_mac_smf.pr.c */
/* Portions of this file copyright 1992-2006 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char wlan_mac_smf_pr_c [] = "MIL_3_Tfile_Hdr_ 115A 30A op_runsim 7 4540BC78 4540BC78 1 apocalypse Jim@Hauser 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 d50 3                                                                                                                                                                                                                                                                                                                                                                                                   ";
#include <string.h>
...
#include <prg_geo.h>
#include "smf_ipc.h" // JPH SMF - shared memory with SMF child process
#include <ip_dgram_sup.h> // JPH SMF - capture IP source address
...
typedef struct WlanT_Mac_Defragmentation_Buffer_Entry
	{		
	int			tx_station_address;		/* Store the address of transmitting station.		*/	 
...
/* JPH SMF - Stear interupt from olsr to SMF child process */
#define SMF						(intrpt_type == OPC_INTRPT_REMOTE)
...
static void			wlan_hlpk_enqueue (Packet* hld_pkptr, int dest_addr, OpT_Packet_Size data_size, Boolean polling);
...
static Boolean		wlan_tuple_find (int sta_addr, int seq_id, int frag_num, int dest_addr, Boolean retry);
...
static void			wlan_sta_addr_register (int bss_idx, int sta_addr, int sta_is_ap, Objid sta_mac_objid, WlanT_Phy_Char_Code phy_char);
...
typedef struct
	...
	int	                    		dcf_destination_addr                            ;	/* Used in storing destination address for DCF transmissions.  */
	int	                    		pcf_destination_addr                            ;	/* Used in storing destination address for CFP transmissions.  */
	...
	Prohandle               		smf_prohandle                                   ;	/* JPH SMF - SMF child process handle */
	smfT_wlan_mem*          		smf_wlan_mem_ptr                                ;	/* JPH SMF - shared memory with SMF child process */
	int                     		my_ip_addr                                      ;	/* JPH SMF - Pass source IP address to SMF */
	} wlan_mac_smf_state;

#define pr_state_ptr            		((wlan_mac_smf_state*) (OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
#define data_tx_rate            		pr_state_ptr->data_tx_rate
#define operational_speed       		pr_state_ptr->operational_speed
#define control_data_rate       		pr_state_ptr->control_data_rate
#define rcvd_frame_drate        		pr_state_ptr->rcvd_frame_drate
#define frag_threshold          		pr_state_ptr->frag_threshold
#define packet_seq_number       		pr_state_ptr->packet_seq_number
#define dcf_destination_addr    		pr_state_ptr->dcf_destination_addr
#define pcf_destination_addr    		pr_state_ptr->pcf_destination_addr
#define packet_frag_number      		pr_state_ptr->packet_frag_number
#define pcf_packet_frag_number  		pr_state_ptr->pcf_packet_frag_number
...
#define mac_client_reassembly_buffer		pr_state_ptr->mac_client_reassembly_buffer
#define fresp_to_send           		pr_state_ptr->fresp_to_send
#define nav_duration            		pr_state_ptr->nav_duration
#define nav_reset_evh           		pr_state_ptr->nav_reset_evh
...
#define global_buffer_drop_handle		pr_state_ptr->global_buffer_drop_handle
#define global_retx_drop_handle 		pr_state_ptr->global_retx_drop_handle
#define global_mac_delay_handle 		pr_state_ptr->global_mac_delay_handle
#define global_network_load_handle		pr_state_ptr->global_network_load_handle
...
#define ap_conn_handle          		pr_state_ptr->ap_conn_handle
#define long_retry_limit        		pr_state_ptr->long_retry_limit
#define short_retry_limit       		pr_state_ptr->short_retry_limit
#define long_retry_count        		pr_state_ptr->long_retry_count
#define short_retry_count       		pr_state_ptr->short_retry_count
...
#define phy_type                		pr_state_ptr->phy_type
#define total_hlpk_size         		pr_state_ptr->total_hlpk_size
#define total_hlpk_num          		pr_state_ptr->total_hlpk_num
#define buffer_drop_pkts_handle 		pr_state_ptr->buffer_drop_pkts_handle
#define buffer_drop_bits_handle 		pr_state_ptr->buffer_drop_bits_handle
#define retx_drop_pkts_handle   		pr_state_ptr->retx_drop_pkts_handle
#define retx_drop_bits_handle   		pr_state_ptr->retx_drop_bits_handle
...
#define beacon_tx_time          		pr_state_ptr->beacon_tx_time
...
#define beacon_tx_count         		pr_state_ptr->beacon_tx_count
#define rem_beacon_tx           		pr_state_ptr->rem_beacon_tx
#define channel_count           		pr_state_ptr->channel_count
#define channel_num             		pr_state_ptr->channel_num
#define first_chan_min_freq     		pr_state_ptr->first_chan_min_freq
#define channel_bandwidth       		pr_state_ptr->channel_bandwidth
#define channel_spacing         		pr_state_ptr->channel_spacing
...
#define ap_peer_info_ptr        		pr_state_ptr->ap_peer_info_ptr
#define peer_info_hash_tbl      		pr_state_ptr->peer_info_hash_tbl
#define debug_mode              		pr_state_ptr->debug_mode
#define mapping_info_mutex      		pr_state_ptr->mapping_info_mutex
#define smf_prohandle           		pr_state_ptr->smf_prohandle
#define smf_wlan_mem_ptr        		pr_state_ptr->smf_wlan_mem_ptr
...
#  define FIN_PREAMBLE_DEC	wlan_mac_smf_state *op_sv_ptr;
#if defined (OPD_PARALLEL)
#  define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((wlan_mac_smf_state *)(sim_context_ptr->_op_mod_state_ptr));
#else
...
wlan_higher_layer_data_arrival (void)
	...
	Ici*					ici_ptr;
	Boolean					polling;
	char					pk_format [128]; // JPH SMF
	IpT_Dgram_Fields*		ip_dgram_fd_ptr; // JPH SMF
	...
	/* JPH SMF - Discover the source IP address and save for SMF my_ip_addr */
	op_pk_format (hld_pkptr, pk_format);	
	if (strcmp (pk_format, "ip_dgram_v4") == 0)
		{
		/*	Get the fields structure from the packet.			*/
		op_pk_nfd_access (hld_pkptr, "fields", &ip_dgram_fd_ptr);				
		/* Determine the source address of the data.			*/
		my_ip_addr = (int)inet_ipv4_address_get (ip_dgram_fd_ptr->src_addr);
		}
	...
	/* The queueing of the packets in the AP is based on the node	*/
	...
	wlan_hlpk_enqueue (hld_pkptr, dest_addr, data_size, polling);
...
static void
wlan_smf_data_arrival (Packet*	hld_pkptr, int	dest_addr)
	{
	OpT_Packet_Size			data_size, frag_size;
	
	/* JPH - Derived from wlan_higher_layer_data_arrival function for SMF forwarding.  */
	...
	FIN (wlan_smf_data_arrival (hld_pkptr, dest_addr));

	/* Get the size of the packet arrived from SMF.		*/
	data_size = op_pk_total_size_get (hld_pkptr);		
	...
	/* Insert the arrived packet in higher layer queue.				*/	
	wlan_hlpk_enqueue (hld_pkptr, dest_addr, data_size, OPC_FALSE);
...
static void
wlan_hlpk_enqueue (Packet* hld_pkptr, int dest_addr, OpT_Packet_Size data_size, Boolean polling)
	{
	char					msg_string [120];
	...
	FIN (wlan_hlpk_enqueue (Packet* hld_pkptr, int dest_addr, data_size, polling));
...
wlan_frame_transmit ()
	...
	if ((wlan_flags->pcf_active == OPC_TRUE) && 
		...
		/* this else if.													*/
			...
			packet_seq_number = (packet_seq_number + 1) % WLANC_SEQ_NUM_WRAP_VALUE;  

			/* Packet fragment number is initialized.	*/	
			pcf_packet_frag_number = 0;				
		...
		/* then dequeue a packet from the higher layer queue.		*/ 
			...
			/* enabled. Broadcast packets are not fragmented regardless	*/
			/* of their sizes.											*/
			if (frag_threshold != -1 && dcf_destination_addr >= 0 && packet_size_dcf > frag_threshold)
				{
				/* Determine number of fragments for the packet	*/
				/* and the size of the last fragment			*/							
			...
			if (rts_threshold != -1 && dcf_destination_addr >= 0 && 
				...
				if (phy_type == WlanC_11g_PHY && wlan_flags->non_erp_present && wlan_flags->polled == OPC_FALSE &&
					...
					if (wlan_flags->cts_to_self || dcf_destination_addr < 0)
						wlan_prepare_frame_to_send (WlanC_Cts);
...
static Boolean
wlan_dest_is_11g_enabled (int dest_mac_addr)
	{
...
wlan_prepare_frame_to_send (WlanT_Mac_Frame_Type frame_type)
	...
	destination_addr = (wlan_flags->pcf_active == OPC_TRUE) ? pcf_destination_addr : dcf_destination_addr;
	...
	if ((wlan_flags->pcf_active == OPC_FALSE) && 
	   ((frame_type == WlanC_Data) || (frame_type == WlanC_Data_Ack)))
		...
		if ((short_retry_count + long_retry_count > 0) && (wlan_transmit_frame_copy_ptr != OPC_NIL))
			...
			if (pk_dhstruct_ptr->fragment_number == 0)
				{
			...
			op_pk_bulk_size_set (wlan_transmit_frame_ptr, (OpT_Packet_Size) (total_plcp_overhead * operational_speed - WLANC_DEFAULT_PLCP_OVERHEAD));
	...
	/* If this is a contention free period and need to send a data/ack/poll.	*/
		...
		if (phy_char_flag != WlanC_Infra_Red)
			op_pk_bulk_size_set (wlan_transmit_frame_ptr, 
								(OpT_Packet_Size) (PLCP_OVERHEAD_CTRL (WLANC_ACK_LENGTH) * tx_data_rate - WLANC_DEFAULT_PLCP_OVERHEAD));
		...
		if (phy_char_flag != WlanC_Infra_Red)
			...
			op_pk_total_size_set (wlan_transmit_frame_ptr, (OpT_Packet_Size) (WLANC_DEFAULT_PLCP_OVERHEAD + WLANC_RTS_LENGTH));
		...
		if (phy_char_flag != WlanC_Infra_Red)
			op_pk_bulk_size_set (wlan_transmit_frame_ptr, 
								(OpT_Packet_Size) (PLCP_OVERHEAD_CTRL (WLANC_CTS_LENGTH) * control_data_rate - WLANC_DEFAULT_PLCP_OVERHEAD));
		...
		if (phy_char_flag != WlanC_Infra_Red)
			...
			op_pk_bulk_size_set (wlan_transmit_frame_ptr,(OpT_Packet_Size) (total_plcp_overhead * tx_data_rate - WLANC_DEFAULT_PLCP_OVERHEAD));
			}
...
wlan_interrupts_process (void)
	...
	/* Handle interrupt from beacon timer.									*/
		...
		sta_mac_addr = intrpt_code >> WlanC_Address_Bit_Shift;
		
		/* Is it an association or deassociation?							*/
		if (intrpt_code & WlanC_Association_Bit)
			...
			peer_info_ptr->seq_num  = WLANC_SEQ_NUM_WRAP_VALUE - 1;
			peer_info_ptr->frag_num = 0;
...
wlan_physical_layer_data_arrival (void)
	...
	if (accept && phy_type == WlanC_11g_PHY)
		...
		if (rcvd_frame_phy_char == WlanC_ERP_OFDM_11g)
			...
			if ((wlan_flags->rcvd_bad_packet == OPC_TRUE) || (wlan_flags->collided_packet == OPC_TRUE))
	...
	if ((wlan_flags->rcvd_bad_packet == OPC_TRUE) || (accept == OPC_FALSE) ||
		...
		if ((accept == OPC_FALSE || wlan_flags->collided_packet == OPC_TRUE || 
	...
	if (rcvd_frame_type & WlanC_Data_Type_Bit)
		...
		if	(((dest_addr == my_address) || (dest_addr < 0)) && data_pkt_received)
			...
			if (wlan_tuple_find (remote_sta_addr, pk_dhstruct_ptr->sequence_number, pk_dhstruct_ptr->fragment_number, 
								 dest_addr, pk_dhstruct_ptr->retry) == OPC_FALSE)
				{
...
static Boolean
wlan_tuple_find (int sta_addr, int seq_id, int frag_num, int dest_addr, Boolean retry)
	{
	...
	FIN (wlan_tuple_find (sta_addr, seq_id, frag_num, dest_addr, retry));
	...
	if (peer_info_hash_tbl != OPC_NIL)
		...
		if (src_sta_info_ptr != OPC_NIL)
			...
			if (retry == OPC_TRUE && src_sta_info_ptr->seq_num == seq_id && src_sta_info_ptr->frag_num >= frag_num)
				...
				src_sta_info_ptr->seq_num  = seq_id;
				src_sta_info_ptr->frag_num = frag_num;
				}
		...
		if (sta_addr == ap_mac_address)
			{
			/* Check whether the latest received frame is a duplicate.	*/
			if (retry == OPC_TRUE && ap_peer_info_ptr->seq_num == seq_id && ap_peer_info_ptr->frag_num >= frag_num)
				duplicate_entry = OPC_TRUE;
			else
				{
				duplicate_entry = OPC_FALSE;
				...
				/* Since this is not a duplicate, update the recorded	*/
				/* information for this source.							*/
				ap_peer_info_ptr->seq_num  = seq_id;
				ap_peer_info_ptr->frag_num = frag_num;
				}
...
wlan_data_process (Packet* seg_pkptr, int dest_addr, int sta_addr, int final_dest_addr,
	...
	if (more_frag == 0)
		...
		/* JPH SMF - Set common memory and invoke SMF child process  */
		/* to handle this INBOUND captured defragmented packet */
		smf_wlan_mem_ptr->pkptr = seg_pkptr;
		smf_wlan_mem_ptr->direction = INBOUND;
		smf_wlan_mem_ptr->tx_addr = sta_addr;
		smf_wlan_mem_ptr->rx_addr = my_address;
		smf_wlan_mem_ptr->tx_ip_addr = my_ip_addr;
		op_pro_invoke (smf_prohandle, smf_wlan_mem_ptr);
		
		/* JPH SMF - if SMF chooses to forward this captured packet, make copy and forward it */
		if (smf_wlan_mem_ptr->forward)
			{
			Packet* forward_pkt = op_pk_copy (seg_pkptr);
			wlan_smf_data_arrival(forward_pkt, dest_addr);
			}
		/* JPH 3/02/06 - if packet is a duplicate multicast packet, don't send it up to the higher layer */
		if (smf_wlan_mem_ptr->is_duplicate)
			{
			if (defrag_ptr != OPC_NIL)
				{
		...
		if (ap_flag == OPC_BOOLINT_ENABLED) 
			...
			if (final_dest_addr == MAC_BROADCAST_ADDR ||
				...
				if (copy_pkptr == OPC_NIL)
					...
					/* First check whether we have sufficient buffer	*/
					...
					if (total_hlpk_size + pkt_size > hld_max_size)
						...
						{
						/* Update the buffer size information and		*/
						/* enqueue the packet.							*/
						total_hlpk_num++;
						total_hlpk_size += pkt_size;
						if (wlan_poll_list_member_find (final_dest_addr) == OPC_TRUE)
							{
							wlan_hlpk_enqueue (copy_pkptr, final_dest_addr, pkt_size, OPC_TRUE);
							}
						else
							{
							wlan_hlpk_enqueue (copy_pkptr, final_dest_addr, pkt_size, OPC_FALSE);
							wlan_flags->data_frame_to_send  = OPC_TRUE;
							}
						}
					}
			...
			if (send_to_higher == OPC_TRUE)
				...
				if (op_ici_attr_set (llc_iciptr, "src_addr", sta_addr) == OPC_COMPCODE_FAILURE)
					{
...
static void
wlan_sta_addr_register (int bssid, int sta_addr, int sta_is_ap, Objid sta_mac_objid, WlanT_Phy_Char_Code phy_char)
	{ 
...
wlan_sta_addr_deregister (int bssid, int sta_addr)
	...
	op_intrpt_schedule_process (bss_info_ptr->ap_prhndl, current_time, 
		(my_address << WlanC_Address_Bit_Shift) + ((phy_char_flag == WlanC_ERP_OFDM_11g) ? WlanC_ERP_Support_Bit : 0));
...
wlan_ap_switch (void)
	...
	/* Set the AP Connectivity stat to the BSS ID of the new AP since we	*/
	/* are now connected to an AP again.									*/
	...
	ap_peer_info_ptr->seq_num  = WLANC_SEQ_NUM_WRAP_VALUE - 1;
	ap_peer_info_ptr->frag_num = 0;
	...
	op_intrpt_schedule_process (my_bss_info_ptr->ap_prhndl, current_time, (my_address << WlanC_Address_Bit_Shift) + 
...
extern "C" {
#endif
	void wlan_mac_smf (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_wlan_mac_smf_init (int * init_block_ptr);
	VosT_Address _op_wlan_mac_smf_alloc (VOS_THREAD_INDEX_ARG_COMMA VosT_Obtype, int);
	void _op_wlan_mac_smf_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_wlan_mac_smf_terminate (OP_SIM_CONTEXT_ARG_OPT);
	void _op_wlan_mac_smf_svar (void *, const char *, void **);
...
void
wlan_mac_smf (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	...
	FIN_MT (wlan_mac_smf ());
		...
		FSM_ENTER ("wlan_mac_smf")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (INIT) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "INIT", "wlan_mac_smf [INIT enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [INIT enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"wlan_mac_smf")


			/** state (INIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "INIT", "wlan_mac_smf [INIT exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [INIT exit execs]", state0_exit_exec)
				{
				...
				// JPH SMF
				/* Allocate common memory and create SMF child process */
				smf_wlan_mem_ptr = op_prg_mem_alloc (sizeof (smfT_wlan_mem));
				smf_prohandle = op_pro_create ("smf_protolib", smf_wlan_mem_ptr);
								
				/* Invoke SMF child process the first time for initialization */
				op_pro_invoke (smf_prohandle, OPC_NIL);
				/* Register SMF child process to handle remote interrupts (from olsr) */
				op_intrpt_type_register (OPC_INTRPT_REMOTE, smf_prohandle);
				// end JPH SMF
				}
			...
			FSM_TRANSIT_FORCE (8, state8_enter_exec, ;, "default", "", "INIT", "BSS_INIT", "wlan_mac_smf [INIT -> BSS_INIT : default / ]")
				/*---------------------------------------------------------*/



			/** state (IDLE) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "IDLE", state1_enter_exec, "wlan_mac_smf [IDLE enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [IDLE enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"wlan_mac_smf")


			/** state (IDLE) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "IDLE", "wlan_mac_smf [IDLE exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [IDLE exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [IDLE trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "READY_TO_TRANSMIT && !wlan_flags->immediate_xmt", "", "IDLE", "DEFER", "wlan_mac_smf [IDLE -> DEFER : READY_TO_TRANSMIT && !wlan_flags->immediate_xmt / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "READY_TO_TRANSMIT && wlan_flags->immediate_xmt", "", "IDLE", "TRANSMIT", "wlan_mac_smf [IDLE -> TRANSMIT : READY_TO_TRANSMIT && wlan_flags->immediate_xmt / ]")
				FSM_CASE_TRANSIT (2, 9, state9_enter_exec, wlan_begin_new_scan ();, "AP_DISCONNECTED", "wlan_begin_new_scan ()", "IDLE", "SCAN", "wlan_mac_smf [IDLE -> SCAN : AP_DISCONNECTED / wlan_begin_new_scan ()]")
				FSM_CASE_TRANSIT (3, 1, state1_enter_exec, ;, "default", "", "IDLE", "IDLE", "wlan_mac_smf [IDLE -> IDLE : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "DEFER", state2_enter_exec, "wlan_mac_smf [DEFER enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [DEFER enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"wlan_mac_smf")


			/** state (DEFER) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "DEFER", "wlan_mac_smf [DEFER exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [DEFER exit execs]", state2_exit_exec)
				{
				/* Lock the mutex that serializes accessing the roaming related		*/
				/* information of this MAC. 										*/
				op_prg_mt_mutex_lock (roam_state_ptr->roam_info_mutex, 0);
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [DEFER trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, ;, "DEFERENCE_OFF", "", "DEFER", "BKOFF_NEEDED", "wlan_mac_smf [DEFER -> BKOFF_NEEDED : DEFERENCE_OFF / ]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, CANCEL_DEF_EVENT;;, "IDLE_AFTER_CFP", "CANCEL_DEF_EVENT;", "DEFER", "IDLE", "wlan_mac_smf [DEFER -> IDLE : IDLE_AFTER_CFP / CANCEL_DEF_EVENT;]")
				FSM_CASE_TRANSIT (2, 2, state2_enter_exec, ;, "default", "", "DEFER", "DEFER", "wlan_mac_smf [DEFER -> DEFER : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (3, "BKOFF_NEEDED", state3_enter_exec, "wlan_mac_smf [BKOFF_NEEDED enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BKOFF_NEEDED enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "BKOFF_NEEDED", "wlan_mac_smf [BKOFF_NEEDED exit execs]")


			/** state (BKOFF_NEEDED) transition processing **/
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BKOFF_NEEDED trans conditions]", state3_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "TRANSMIT_FRAME", "", "BKOFF_NEEDED", "TRANSMIT", "wlan_mac_smf [BKOFF_NEEDED -> TRANSMIT : TRANSMIT_FRAME / ]")
				FSM_CASE_TRANSIT (1, 5, state5_enter_exec, ;, "PERFORM_BACKOFF", "", "BKOFF_NEEDED", "BACKOFF", "wlan_mac_smf [BKOFF_NEEDED -> BACKOFF : PERFORM_BACKOFF / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (4, "TRANSMIT", state4_enter_exec, "wlan_mac_smf [TRANSMIT enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [TRANSMIT enter execs]", state4_enter_exec)
				{
				/** In this state following intrpts can occur:     				**/
				/** 1. Data arrival from application layer.			        	**/
				/** 2. Frame (DATA,ACK,RTS,CTS) rcvd from PHY layer.			**/
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"wlan_mac_smf")


			/** state (TRANSMIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "TRANSMIT", "wlan_mac_smf [TRANSMIT exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [TRANSMIT exit execs]", state4_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [TRANSMIT trans conditions]", state4_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 6, state6_enter_exec, ;, "TRANSMISSION_COMPLETE", "", "TRANSMIT", "FRM_END", "wlan_mac_smf [TRANSMIT -> FRM_END : TRANSMISSION_COMPLETE / ]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, ;, "default", "", "TRANSMIT", "TRANSMIT", "wlan_mac_smf [TRANSMIT -> TRANSMIT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (5, "BACKOFF", state5_enter_exec, "wlan_mac_smf [BACKOFF enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BACKOFF enter execs]", state5_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (11,"wlan_mac_smf")


			/** state (BACKOFF) exit executives **/
			FSM_STATE_EXIT_UNFORCED (5, "BACKOFF", "wlan_mac_smf [BACKOFF exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BACKOFF exit execs]", state5_exit_exec)
				{
				/* Lock the mutex that serializes accessing the roaming related		*/
				/* information of this MAC. 										*/
				op_prg_mt_mutex_lock (roam_state_ptr->roam_info_mutex, 0);
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BACKOFF trans conditions]", state5_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "PERFORM_TRANSMIT", "", "BACKOFF", "TRANSMIT", "wlan_mac_smf [BACKOFF -> TRANSMIT : PERFORM_TRANSMIT / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, wlan_schedule_deference ();;, "BACK_TO_DEFER", "wlan_schedule_deference ();", "BACKOFF", "DEFER", "wlan_mac_smf [BACKOFF -> DEFER : BACK_TO_DEFER / wlan_schedule_deference ();]")
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, ;, "BACK_TO_IDLE", "", "BACKOFF", "IDLE", "wlan_mac_smf [BACKOFF -> IDLE : BACK_TO_IDLE / ]")
				FSM_CASE_TRANSIT (3, 9, state9_enter_exec, wlan_begin_new_scan ();, "SCAN_AFTER_CW", "wlan_begin_new_scan ()", "BACKOFF", "SCAN", "wlan_mac_smf [BACKOFF -> SCAN : SCAN_AFTER_CW / wlan_begin_new_scan ()]")
				FSM_CASE_TRANSIT (4, 5, state5_enter_exec, ;, "default", "", "BACKOFF", "BACKOFF", "wlan_mac_smf [BACKOFF -> BACKOFF : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (6, "FRM_END", state6_enter_exec, "wlan_mac_smf [FRM_END enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [FRM_END enter execs]", state6_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (6, "FRM_END", "wlan_mac_smf [FRM_END exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [FRM_END exit execs]", state6_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [FRM_END trans conditions]", state6_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "FRM_END_TO_IDLE", "", "FRM_END", "IDLE", "wlan_mac_smf [FRM_END -> IDLE : FRM_END_TO_IDLE / ]")
				FSM_CASE_TRANSIT (1, 7, state7_enter_exec, ;, "WAIT_FOR_FRAME", "", "FRM_END", "WAIT_FOR_RESPONSE", "wlan_mac_smf [FRM_END -> WAIT_FOR_RESPONSE : WAIT_FOR_FRAME / ]")
				FSM_CASE_TRANSIT (2, 2, state2_enter_exec, ;, "FRM_END_TO_DEFER", "", "FRM_END", "DEFER", "wlan_mac_smf [FRM_END -> DEFER : FRM_END_TO_DEFER / ]")
				FSM_CASE_TRANSIT (3, 9, state9_enter_exec, wlan_begin_new_scan ();, "FRM_END_TO_SCAN", "wlan_begin_new_scan ()", "FRM_END", "SCAN", "wlan_mac_smf [FRM_END -> SCAN : FRM_END_TO_SCAN / wlan_begin_new_scan ()]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (7, "WAIT_FOR_RESPONSE", state7_enter_exec, "wlan_mac_smf [WAIT_FOR_RESPONSE enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [WAIT_FOR_RESPONSE enter execs]", state7_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (15,"wlan_mac_smf")


			/** state (WAIT_FOR_RESPONSE) exit executives **/
			FSM_STATE_EXIT_UNFORCED (7, "WAIT_FOR_RESPONSE", "wlan_mac_smf [WAIT_FOR_RESPONSE exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [WAIT_FOR_RESPONSE exit execs]", state7_exit_exec)
				{
				/* Lock the mutex that serializes accessing the roaming related	*/
				/* information of this MAC. 									*/
				op_prg_mt_mutex_lock (roam_state_ptr->roam_info_mutex, 0);
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [WAIT_FOR_RESPONSE trans conditions]", state7_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 6, state6_enter_exec, ;, "FRAME_TIMEOUT || FRAME_RCVD", "", "WAIT_FOR_RESPONSE", "FRM_END", "wlan_mac_smf [WAIT_FOR_RESPONSE -> FRM_END : FRAME_TIMEOUT || FRAME_RCVD / ]")
				FSM_CASE_TRANSIT (1, 7, state7_enter_exec, ;, "default", "", "WAIT_FOR_RESPONSE", "WAIT_FOR_RESPONSE", "wlan_mac_smf [WAIT_FOR_RESPONSE -> WAIT_FOR_RESPONSE : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (8, "BSS_INIT", state8_enter_exec, "wlan_mac_smf [BSS_INIT enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BSS_INIT enter execs]", state8_enter_exec)
				{
				/* Schedule a self interrupt to wait for mac interface 	*/
				/* to move to next state after registering				*/
				op_intrpt_schedule_self (op_sim_time (), 0);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"wlan_mac_smf")


			/** state (BSS_INIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "BSS_INIT", "wlan_mac_smf [BSS_INIT exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [BSS_INIT exit execs]", state8_exit_exec)
				{
				/* Obtain the values assigned to the various attributes				*/
				op_ima_obj_attr_get (my_objid, "Wireless LAN Parameters", &wlan_params_comp_attr_objid);
				params_attr_objid = op_topo_child (wlan_params_comp_attr_objid, OPC_OBJTYPE_GENERIC, 0);
				...
				if (record_handle_list_size <= 2)
					peer_info_hash_tbl = prg_bin_hash_table_create (2, sizeof (int));
				else
					peer_info_hash_tbl = prg_bin_hash_table_create ((int) floor (log (record_handle_list_size * 1.5) / log (2)) + 1, sizeof (int));
				...
				for (i_cnt = 0; i_cnt < record_handle_list_size; i_cnt++ )
					...
					op_ima_obj_attr_get (mac_objid, "Address", &sta_addr);
			...
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "BSS_INIT", "IDLE", "wlan_mac_smf [BSS_INIT -> IDLE : default / ]")
				/*---------------------------------------------------------*/



			/** state (SCAN) enter executives **/
			FSM_STATE_ENTER_UNFORCED (9, "SCAN", state9_enter_exec, "wlan_mac_smf [SCAN enter execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [SCAN enter execs]", state9_enter_exec)
				{
				/* Record the state name if running in debug mode.			*/
				if (debug_mode)
					{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (19,"wlan_mac_smf")


			/** state (SCAN) exit executives **/
			FSM_STATE_EXIT_UNFORCED (9, "SCAN", "wlan_mac_smf [SCAN exit execs]")
				FSM_PROFILE_SECTION_IN ("wlan_mac_smf [SCAN exit execs]", state9_exit_exec)
				{
				/* Lock the mutex that serializes accessing the roaming related		*/
				/* information of this MAC. 										*/
				op_prg_mt_mutex_lock (roam_state_ptr->roam_info_mutex, 0);
			...
			FSM_PROFILE_SECTION_IN ("wlan_mac_smf [SCAN trans conditions]", state9_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "AP_CONNECTED && !DATA_FRAME_TO_TX", "", "SCAN", "IDLE", "wlan_mac_smf [SCAN -> IDLE : AP_CONNECTED && !DATA_FRAME_TO_TX / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, wlan_schedule_deference ();, "AP_CONNECTED && DATA_FRAME_TO_TX", "wlan_schedule_deference ()", "SCAN", "DEFER", "wlan_mac_smf [SCAN -> DEFER : AP_CONNECTED && DATA_FRAME_TO_TX / wlan_schedule_deference ()]")
				FSM_CASE_TRANSIT (2, 9, state9_enter_exec, ;, "default", "", "SCAN", "SCAN", "wlan_mac_smf [SCAN -> SCAN : default / ]")
				}
				/*---------------------------------------------------------*/
...
void
_op_wlan_mac_smf_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
...
void
_op_wlan_mac_smf_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = __LINE__;
#endif

	FIN_MT (_op_wlan_mac_smf_terminate ())
...
/* local variable prs_ptr in _op_wlan_mac_smf_svar function. */
#undef intrpt_type
#undef intrpt_code
#undef my_address
#undef my_objid
...
VosT_Obtype
_op_wlan_mac_smf_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_wlan_mac_smf_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (wlan_mac_smf)",
		sizeof (wlan_mac_smf_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_wlan_mac_smf_alloc (VOS_THREAD_INDEX_ARG_COMMA VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wlan_mac_smf_state * ptr;
	FIN_MT (_op_wlan_mac_smf_alloc (obtype))

	ptr = (wlan_mac_smf_state *)Vos_Alloc_Object_MT (VOS_THREAD_INDEX_COMMA obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "wlan_mac_smf [INIT enter execs]";
#endif
		}
...
void
_op_wlan_mac_smf_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	wlan_mac_smf_state		*prs_ptr;

	FIN_MT (_op_wlan_mac_smf_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (wlan_mac_smf_state *)gen_ptr;
	...
	if (strcmp ("smf_prohandle" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->smf_prohandle);
		FOUT
		}
	if (strcmp ("smf_wlan_mem_ptr" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->smf_wlan_mem_ptr);
		FOUT
		}
