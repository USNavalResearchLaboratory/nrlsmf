...
typedef struct WlanT_Hld_List_Elem
	...
	int			origination_address;	/* Packet's original source address (used by APs).	*/
...
static void			wlan_hlpk_enqueue (Packet* hld_pkptr, int dest_addr, int orig_src_addr, int protocol_type, OpT_Packet_Size data_size, Boolean polling);
...
static Boolean		wlan_tuple_find (int sta_addr, int seq_cntl, int dest_addr, Boolean retry);
static void			wlan_data_process (Packet* seg_pkptr, int dest_addr, int sta_addr, int final_dest_addr, int protocol_type, int frag_num, int more_frag, OpT_Packet_Id pkt_id);
...
typedef struct
	...
	int	                    		dcf_orig_source_addr                            ;	/* Stores the address of the source STA for packets relayed by AP. */
	...
	int	                    		pcf_orig_source_addr                            ;	/* Stores the address of the source STA for packets relayed by AP */
	                        		                                                	/* during CFPs.                                                   */
...
#define smf_prohandle           		op_sv_ptr->smf_prohandle
#define smf_wlan_mem_ptr        		op_sv_ptr->smf_wlan_mem_ptr
...
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((wlan_mac_smf_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
wlan_smf_data_arrival (Packet*	hld_pkptr, int	dest_addr)
	{
	OpT_Packet_Size			data_size, frag_size;
	int protocol_type = 0;  /* Don't know what higher layer protocol originated this packet. */
	...
	/* Insert the arrived packet in higher layer queue.				*/	
	wlan_hlpk_enqueue (hld_pkptr, dest_addr, my_address, protocol_type, data_size, OPC_FALSE);
...
static void
wlan_hlpk_enqueue (Packet* hld_pkptr, int dest_addr, int orig_src_addr, int protocol_type, OpT_Packet_Size data_size, Boolean polling)
	{
	char					msg_string [120];
...
wlan_physical_layer_data_arrival (void)
	...
	if (rcvd_frame_type & WLANC_DATA_TYPE_BIT)
		...
		if	(((dest_addr == my_address) || 
			...
			if (wlan_tuple_find (remote_sta_addr, pk_dhstruct_ptr->sequence_control, dest_addr, pk_dhstruct_ptr->retry) == OPC_FALSE)
				{
				...
				wlan_data_process (seg_pkptr, dest_addr, remote_sta_addr, pk_dhstruct_ptr->address3, pk_dhstruct_ptr->hl_protocol,
					pk_dhstruct_ptr->sequence_control & WLANC_FRAG_NUM_BIT_MASK, pk_dhstruct_ptr->more_frag, data_pkt_id);
				}
			else
				{
...
static Boolean
wlan_tuple_find (int sta_addr, int seq_cntl, int dest_addr, Boolean retry)
	{
...
static void
wlan_data_process (Packet* seg_pkptr, int dest_addr, int sta_addr, int final_dest_addr, int protocol_type,
...
wlan_sta_addr_deregister (int bssid, int sta_addr)
	...
	op_intrpt_schedule_process (bss_info_ptr->ap_prhndl, current_time, 
		(sta_addr << WLANC_ADDRESS_BIT_SHIFT) + ((phy_char_flag == WlanC_ERP_OFDM_11g) ? WLANC_ERP_SUPPORT_BIT : 0));
...
extern "C" {
	...
	VosT_Address _op_wlan_mac_smf_alloc (VosT_Obtype, int);
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_EXIT_UNFORCED (4, "TRANSMIT", "wlan_mac_smf [TRANSMIT exit execs]")
				...
				if (op_intrpt_type () == OPC_INTRPT_STAT)
					...
					/* If we completed the transmission then reset the transmitter	*/
					/* flag.														*/
					else if (intrpt_code == TRANSMITTER_BUSY_INSTAT)
						{
						wlan_flags->transmitter_busy = OPC_FALSE;
						
						/* Also update the receiver idle time, since with the end	*/
						/* of our transmission, the medium may become idle again.	*/
						/* If the transmission requires 11g signal extension, model	*/
						/* the 6-usec no-transmission duration of signal extension	*/
						/* by adjusting the receiver idle time accordingly. This is	*/
						/* needed to remain synchronized with other STAs in our	BSS	*/
						/* following a transmission that doesn't require an			*/
						/* acknowledgement like data frames with "broadcast"		*/
					...
					{
					/* While transmitting, we received a packet from physical		*/
					/* layer. Mark the packet as bad.								*/
					wlan_flags->rcvd_bad_packet = OPC_TRUE;
...
VosT_Address
_op_wlan_mac_smf_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	...
	ptr = (wlan_mac_smf_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
