...
#include "animInfo.h"  // JPH 11/15/06 - packet animation
...
#define TTL_TO			((op_intrpt_type () == OPC_INTRPT_SELF) && (op_intrpt_code() == TTL_ANIM_TO))
...
class smf_protolib_state
	...
	public:
		...
		Andid	                  		ttl_did                                         ;	/* JPH 11/20/06 - packet animation */
		...
		Evhandle	               		ttl_timeout_ev                                  ;	/* JPH 12/6/06 - packet animation */
...
#define bits_frwd_stathandle    		op_sv_ptr->bits_frwd_stathandle
#define pkts_frwd_stathandle    		op_sv_ptr->pkts_frwd_stathandle
#define olsr_packet_capture     		op_sv_ptr->olsr_packet_capture
#define invoke_prohandle        		op_sv_ptr->invoke_prohandle
#define seq_word_size           		op_sv_ptr->seq_word_size
#define window_size             		op_sv_ptr->window_size
#define window_past_max         		op_sv_ptr->window_past_max
#define duplicate_filtering     		op_sv_ptr->duplicate_filtering
#define tx_did                  		op_sv_ptr->tx_did
#define rx_did                  		op_sv_ptr->rx_did
#define ttl_did                 		op_sv_ptr->ttl_did
#define my_pnum                 		op_sv_ptr->my_pnum
#define tx_timeout_ev           		op_sv_ptr->tx_timeout_ev
#define rx_timeout_ev           		op_sv_ptr->rx_timeout_ev
#define ttl_timeout_ev          		op_sv_ptr->ttl_timeout_ev
...
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((smf_protolib_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
static void smf_init ()
	...
	ttl_did = -1;  // JPH 11/20/06 - packet animation
...
void OpnetSmfProcess::OnPktReceive(smfT_wlan_mem* mem)
	...
	Packet*						mgen_pkptr;
	int 						port;
	UdpT_Dgram_Fields*			udp_fields_ptr;
	TcpT_Seg_Fields*			tcp_fields_ptr;	
	...
	// JPH 11/15/06 - packet animation
	/*	Get the data structure from IP packet.			*/
	transport_pkptr = ip_dgram_data_pkt_get(ip_pkptr);
	op_pk_format (transport_pkptr, pk_format);	
	if (strcmp (pk_format, "udp_dgram_v2") == 0 && !dstIp.IsBroadcast())  // Only display unicast UDP packet arrival.
		{																  // Broadcast UPD packets will be displayed if forwarded.
		op_pk_nfd_access (transport_pkptr, "fields", &udp_fields_ptr);
		port = udp_fields_ptr->dest_port;
		for (int i=0; i<mgen_udp_port_cnt; i++)
			if (port == mgen_udp_port[i])
				{
				char* buffer;
				MgenMsg msg;
				op_pk_nfd_get (transport_pkptr, "data", &mgen_pkptr);
				double pk_size = (double) op_pk_total_size_get (mgen_pkptr);
				op_pk_fd_access_ptr(mgen_pkptr,0,(void**)&buffer);
				...
				if (animFlag)
					...
					if (op_ev_valid (ttl_timeout_ev))
						op_ev_cancel (ttl_timeout_ev);
					/* Draw flow id at receiving node's position. */
					char flowId_str[4], ttl_str[4];
					sprintf(flowId_str,"%d",flowId);
					sprintf(ttl_str,"%d",ttl);
					rx_did = op_anim_igp_text_draw (olsr_vid, 
						OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_COLOR_YELLOW |
						OPC_ANIM_ALIGNH_RIGHT | OPC_ANIM_ALIGNV_CENTER,
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy+15, flowId_str);
					ttl_did = op_anim_igp_text_draw (olsr_vid, 
						OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_COLOR_PURPLE |
						OPC_ANIM_ALIGNH_RIGHT | OPC_ANIM_ALIGNV_CENTER,
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy+27, ttl_str);
					rx_timeout_ev = op_intrpt_schedule_self (op_sim_time () + RX_DISPLAY_TIME, RX_ANIM_TO);
					ttl_timeout_ev = op_intrpt_schedule_self (op_sim_time () + TTL_DISPLAY_TIME, TTL_ANIM_TO);
					}
				op_pk_nfd_set (transport_pkptr, "data", mgen_pkptr);
				break;
				}
		}
	else if (strcmp (pk_format, "tcp_seg_v2") == 0)
		{
		op_pk_nfd_access (transport_pkptr, "fields", &tcp_fields_ptr);
		port = tcp_fields_ptr->dest_port;
		for (int i=0; i<mgen_tcp_port_cnt; i++)
			if (port == mgen_tcp_port[i])
				{
				char* buffer;
				MgenMsg msg;
				op_pk_nfd_get (transport_pkptr, "data", &mgen_pkptr);
				...
				if (animFlag)
					...
					rx_did = op_anim_igp_text_draw (olsr_vid, 
						...
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy+15, flowId_str);
					ttl_did = op_anim_igp_text_draw (olsr_vid, 
						OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_COLOR_GREEN |
						OPC_ANIM_ALIGNH_RIGHT | OPC_ANIM_ALIGNV_CENTER,
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy+27, ttl_str);
					rx_timeout_ev = op_intrpt_schedule_self (op_sim_time () + RX_DISPLAY_TIME, RX_ANIM_TO);
					}
	...
	op_pk_nfd_set (ip_pkptr, "data", transport_pkptr);
		
	// JPH - end packet animation
			...
			if (port == mgen_udp_port[i])
				...
				if (animFlag)
					...
					tx_did = op_anim_igp_text_draw (olsr_vid, 
						OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_COLOR_PLUM |
						OPC_ANIM_ALIGNH_LEFT | OPC_ANIM_ALIGNV_CENTER,
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy+20, flowId_str);
					ttl_did = op_anim_igp_text_draw (olsr_vid, 
						OPC_ANIM_RETAIN | OPC_ANIM_PIXOP_XOR | OPC_ANIM_COLOR_GREEN |
						OPC_ANIM_ALIGNH_RIGHT | OPC_ANIM_ALIGNV_CENTER,
						olsrNode[my_pnum]->vx, olsrNode[my_pnum]->vy+27, ttl_str);
...
void OpnetSmfProcess::OnControlMsg(char* buffer, int len)
    ...
    if (!strncmp(cmd, "smfServerStart", cmdLen))
		...
		// LP 10-23-06 - replaced - The error msg should state that it's generated
		// from OpnetsmfProcess:: OnControlMsg instead of SmfApp:OnControlMsg
		
        // smf_warn("SmfApp::OnControlMsg(smfServerStart) not expected");
         smf_warn("OpnetSmfProcess::OnControlMsg(smfServerStart) not expected");
		...
		// from OpnetsmfProcess:: OnControlMsg instead of SmfApp:OnControlMsg
           //  DMSG(0, "SmfApp::OnControlMsg(forwardMac) error: selector list too long!\n");
			DMSG(0, "OpnetSmfProcess::OnControlMsg(forwardMac) error: selector list too long!\n");
		...
		// from OpnetsmfProcess:: OnControlMsg instead of SmfApp:OnControlMsg
			...
			DMSG(0, "OpnetSmfProcess::OnControlMsg(symetricMac) error: symmetric list too long!\n");           // (TBD) record this error indication permanently
            argLen = SELECTOR_LIST_LEN_MAX;
			}
		...
		// from OpnetsmfProcess:: OnControlMsg instead of SmfApp:OnControlMsg
            // DMSG(0, "SmfApp::OnControlMsg(mneBlock) error: mac list too long!\n");
             DMSG(0, "OpnetSmfProcess::OnControlMsg(mneBlock) error: mac list too long!\n");
            // (TBD) record this error indication permanently
            argLen = SELECTOR_LIST_LEN_MAX;
            }
		...
		// from OpnetsmfProcess:: OnControlMsg instead of SmfApp:OnControlMsg
         ...
         DMSG(0, "OpnetSmfProcess::OnControlMsg() invalid command: \"%s\"\n", cmd);
        }
    }   
...
bool OpnetSmfProcess::OnCommand(const char* cmd, const char* val)
{
    ...
    if(CMD_INVALID == type)
    {
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
	
       //  DMSG(0, "SmfApp::OnCommand() invalid command: %s\n", cmd);
		DMSG(0, "OpnetSmfProcess::OnCommand() invalid command: %s\n", cmd);
 		// end LP
        return false;   
    }
    ...
    if ((CMD_ARG == type) && !val)
    {
		...
		DMSG(0, "OpnetSmfProcess::OnCommand(%s) missing argument\n", cmd);
		// end LP
        return false;
    }
    ...
    else if (!strncmp("ipv6", cmd, len))
    {
		...
		DMSG(0, "OpnetSmfProcess::OnCommand(ipv6) error opening IPv6 detour\n");
            return false;
        }
#endif // _PROTO_DETOUR
        ipv6_enabled = true;
    ...
    else if (!strncmp("interface", cmd, len))
    {
        ...
        {
 		// LP 10-23-06 - replaced. The Debug msg should state smf_protolib.pr.m, instead 
	    //     of nrlsmf.ex.cpp()
        //     DMSG(0, "nrlsmf: invalid <interfaceName>\n");
		DMSG(0, "smf_protolib.pr.m: invalid <interfaceName>\n");
            return false;
        }
        ...
        //   DMSG(0, "nrlsmf: error getting interface MAC address\n");
		   DMSG(0, "smf_protolib.pr.m: error getting interface MAC address\n");
            return false;
        }
	    ...
	    //     of SmfApp:OnCommand()
         //          DMSG(1, "SmfApp::OnCommand() ProtoSocket::GetInterfaceAddress(IPv6) failed\n");
                    DMSG(1, "OpnetSmfProcess::OnCommand ProtoSocket::GetInterfaceAddress(IPv6) failed\n");
                routeMgr->Close();
            }
            else
            {
        ...
        else if (!ProtoSocket::GetInterfaceAddress(ifName, ProtoAddress::IPv6, my_ip6_addr))
        {
 		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
                DMSG(1, "OpnetSmfProcess::OnCommand() ProtoRouteMgr::OPen() failed\n");
        }
#endif // HAVE_IPV6
	    ...
	    if (!cap_rcvr->IsOpen())
        {
  		...
  		// LP 10-23-06 - replaced. The Debug msg should state smf_protolib.pr.m, instead 
	    //     of nrlsmf.ex.cpp()
                DMSG(0, "smf_protolib.pr.mf: error opening ProtoCap rcvr\n");
                return false;
	        }
   		// LP 10-23-06 - replaced. The Debug msg should state smf_protolib.pr.m, instead 
	    //     of nrlsmf.ex.cpp()
           DMSG(0, "smf_protolib.pr.m: listening on interface \"%s\" ...\n", ifName);
	    }
#endif  // OPNET
    }
    ...
    else if (!strncmp("resequence", cmd, len))
    {
		...
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
                DMSG(0, "OpnetSmfProcess::OnCommand(resequence) new ProtoDetour error: %s\n",
                        GetErrorString());
                return false;
            }
		...
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
                DMSG(0, "OpnetSmfProcess::OnCommand(resequence) error opening IPv4 detour\n");
                return false;
            }
#ifdef HAVE_IPV6
		...
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
                DMSG(0,  "OpnetSmfProcess::OnCommand(resequence) error opening IPv6 detour\n"); 
                return false; 
            }
#endif // HAVE_IPV6
#endif // _PROTO_DETOUR
		...
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
            DMSG(0, "OpnetSmfProcess::OnCommand(resequence) invalid argument: %s\n", val);
            return false;
        }
    }      
    else if (!strncmp("defaultForward", cmd, len))
    {
		...
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
            DMSG(0, "OpnetSmfProcess::OnCommand(defaultForward) invalid argument: %s\n", val);
            return false;
        }
    }    
    ...
    else if (!strncmp("log", cmd, len))
    {
		...
		// LP 10-23-06 - replaced. The Debug msg should staate OpnetSmfProcess::OnCommand, instead 
	    //     of SmfApp:OnCommand()
            DMSG(0, "OpnetSmfProcess::OnCommand(log) error opening file:\n", GetErrorString());
            return false;
        }
    }
    return true;
	...
	try
		...
		FSM_BLOCK_SWITCH
			...
			FSM_TRANSIT_SWITCH
				...
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, smf_rx_erase();, "RX_TO", "smf_rx_erase()", "idle", "idle", "smf_protolib [idle -> idle : RX_TO / smf_rx_erase()]")
				FSM_CASE_TRANSIT (3, 1, state1_enter_exec, smf_tx_erase();, "TX_TO", "smf_tx_erase()", "idle", "idle", "smf_protolib [idle -> idle : TX_TO / smf_tx_erase()]")
				FSM_CASE_TRANSIT (4, 1, state1_enter_exec, smf_ttl_erase();, "TTL_TO", "smf_ttl_erase()", "idle", "idle", "smf_protolib [idle -> idle : TTL_TO / smf_ttl_erase()]")
				FSM_CASE_TRANSIT (5, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "smf_protolib [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
...
smf_protolib_state::operator new (size_t)
	...
	new_ptr = Vos_Alloc_Object (smf_protolib_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
VosT_Address
_op_smf_protolib_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
...
_op_smf_protolib_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	...
	if (strcmp ("ttl_did" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->ttl_did);
		FOUT
		}
	...
	if (strcmp ("ttl_timeout_ev" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->ttl_timeout_ev);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
